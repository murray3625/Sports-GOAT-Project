...
f2 <- ggplot(data = data.agg, 
  ...
  scale_y_continuous(limits = c(0, 12), breaks = seq(0, 15, by = 1))+
  geom_line() +
  labs(x = "Year", 
       y = "Number of Potential GOATs", 
...
tbl.1 <- as.data.frame(data.2)%>%
  ...
  kable(format = "markdown", align = "cc", caption = "Average Career Length by Sport") %>%
...
Average # of GOAT candiates per year conditional on not having 0
...
ggplot(data = data.combined.2, 
  ...
  labs(x = "Average Career Length in Years", 
       y = "# Years with no Potential GOATs", 
