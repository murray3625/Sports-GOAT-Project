...
### Average Career Length by Sport (in Years)
...
tbl.1 <- as.data.frame(data.2)%>%
  arrange(., desc(avg.length)) %>%
  mutate (avg.length = round(avg.length, 1)) %>%
  rename("Average Career Length" = avg.length)%>%
...
tbl.2.data <- cbind(data.2[, 1], lengths)
colnames(tbl.2.data) <- colnames(data.2)
...
tbl.2 <- kable(dt_footnote, format = "markdown", align = "cc", caption = "Average Career Length by Sport") %>%
  kable_styling(font_size = 8.5, latex_options = c("hold_position"),
                full_width = FALSE, position = "float_left") %>%
...
data.combined.3 <- data.combined.2
...
***Figure 11*** - One would expect to see a **negative** relationship between the average career length and the number of years with ZERO potential GOATs. 
  ...
  * Above, I will classify **basketball, football, hockey, and soccer** as **contact** sports; conversely, **baseball, cricket, golf, and tennis** will be classified as **limited contact or non contact**. 
...
# Average career length for each sports

* Basketball (NBA): 4.8 years
  * https://www.espn.com/nba/story/_/page/nextforplayers-111114/nba-players-do-next 
  * 4.5 years from this article - https://www.nba.com/nuggets/features/junior_bridgeman_20100610.html
* Football (NFL): 3.3 years
  * https://www.espn.com/blog/nflnation/post/_/id/207780/current-and-former-nfl-players-in-the-drivers-seat-after-completing-mba-program
...
Repeat Table 1 and Figure 11 - in Table 1, add a footnote with the sources
